...
// Lectura de archivo input 2D
	...
	for(int j=0;j<ny;j++)	file >> posY[j];
	for(int j=0;j<ny;j++)	file >> bat[j];
	for(int k=0;k<nz;k++)	file >> posZ[k];
...
// Tercera fila: posY
	for(int j=0;j<ny;j++)	file3D << posY[j] << '\t';
	file3D << '\n';

// Cuarto bloque: batimetria
	for(int j=0;j<ny;j++)	file3D << bat[j] << '\t';
	file3D << '\n';

// Quinta fila: posZ
	for(int k=0;k<nz;k++)	file3D << posZ[k] << '\t';
	file3D << '\n';
...
void data_2d3d :: corr_fn_2d3d() {
        ...
        file3D << '\n';
        if(num_x==1)file3D << xmin;
        if(num_x==2)file3D << xmin << '\t' << xmax;
        file3D << '\n';
        file3D << ymin << '\t' << ymax;
        file3D << '\n';
