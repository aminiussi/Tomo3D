...
for o in $(ls $srcdir/gold/*smesh*); do
    fname=$(basename $o)
    $check_output $srcdir/gold/$fname $bindir/output/$fname --format=smesh --nb-errors=50 --diff=$bindir/output/$fname.diff --max-diff 0.0005
    ostatus=$?
...
for o in $(ls $srcdir/gold/*tres*); do
    fname=$(basename $o)
    $check_output $srcdir/gold/$fname $bindir/output/$fname --format=tres --nb-error=50 --diff=$bindir/output/$fname.diff --max-diff 0.008
    ostatus=$?
    if [ $ostatus -ne 0 ]; then
	let tres_failed++
	echo "got mistmatch on output $fname"
...
for o in $(ls $srcdir/gold/*.log); do
    fname=$(basename $o)
    $check_output --format log $srcdir/gold/$fname $bindir/output/$fname --format=log --diff=$bindir/output/$fname.diff
    ostatus=$?
    if [ $ostatus -ne 0 ]; then
	let log_failed++
	echo "got mistmatch on output $fname"
